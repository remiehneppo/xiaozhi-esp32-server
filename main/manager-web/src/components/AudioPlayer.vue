...
// Trả về đường dẫn icon tương ứng theo trạng thái âm lượng
...
const toggleMute = () => {
  ...
  // Nếu được bật tiếng và âm lượng bằng 0, hãy khôi phục âm lượng mặc định
  if (!isMuted.value && volume.value === 0) {
    volume.value = 0.5
...
// Chức năng kéo thanh tiến trình
...
// Điều khiển hiển thị/ẩn thanh trượt âm lượng
...
const updateSliderPosition = () => {
  nextTick(() => {
    ...
    // Tính toán vị trí liên quan đến khung nhìn
    const left = buttonRect.left + window.scrollX
    const top = buttonRect.top + window.scrollY

    // Đặt nó ngay giữa nút, căn giữa theo chiều dọc
