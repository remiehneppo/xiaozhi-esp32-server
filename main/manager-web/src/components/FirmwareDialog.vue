...
export default {
  ...
  computed: {
    isTypeDisabled() {
      // Nếu có id, điều đó có nghĩa là nó đang ở chế độ chỉnh sửa và việc chọn loại bị tắt.
      return !!this.form.id
    }
  },
  created() {
    // Xóa cuộc gọi getDictDataByType
  },
  watch: {
    visible(val) {
  ...
  methods: {
    ...
    handleSubmit() {
      this.$refs.form.validate(valid => {
        if (valid) {
          // Nếu đây là chế độ mới và không có tệp nào được tải lên thì sẽ xuất hiện lỗi.
          if (!this.form.id && !this.form.firmwarePath) {
            this.$message.error(this.$t('firmwareDialog.requiredFirmwareFile'))
            return
          }
          // Sau khi gửi thành công, logic đóng hộp thoại sẽ được chuyển cho thành phần cha mẹ để xử lý.
    ...
    handleUpload(options) {
      ...
      // Sử dụng setTimeout để triển khai đơn giản0-50%chuyển tiếp
      const timer = setTimeout(() => {
        if (this.uploadProgress < 50) {  // Chỉ khi tiến độ ít hơn50%Chỉ đặt khi
      ...
      Api.ota.uploadFirmware(file, (res) => {
        ...
        if (res.code === 0) {
          ...
          // Ẩn thanh tiến trình sau 2 giây trễ
          setTimeout(() => {
        ...
        if (progressEvent.total) {
          ...
          // Chỉ khi tiến độ lớn hơn50%Chỉ cập nhật khi
          if (progress > 50) {
            this.uploadProgress = progress
          }
          // Nếu quá trình tải lên hoàn tất nhưng chưa nhận được phản hồi thành công, hãy tiếp tục hiển thị thanh tiến trình.
          if (progress === 100) {
            this.uploadStatus = ''
          }
        }
    ...
    handleOpen() {
      ...
      if (!this.form.id) {  // Chỉ reset khi mới
        this.form.firmwarePath = ''
        this.form.size = 0
      }
      // Đặt lại thành phần tải lên bất kể chế độ chỉnh sửa hay không
      this.$nextTick(() => {
        if (this.$refs.upload) {
