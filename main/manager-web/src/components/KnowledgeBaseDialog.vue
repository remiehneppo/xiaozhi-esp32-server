...
export default {
  ...
  watch: {
    visible(val) {
      ...
      if (val) {
        // Tải danh sách model RAG khi hộp thoại hiển thị
        this.loadRAGModels();

        // Nếu cơ sở kiến ​​thức mới được thêm vào và ragModelId không được đặt thì mô hình RAG đầu tiên sẽ được chọn theo mặc định.
    ...
    // Giám sát các thay đổi trong danh sách mô hình RAG để đảm bảo rằng các giá trị mặc định được đặt chính xác khi thực hiện bổ sung mới.
    ragModels(newModels) {
  ...
  methods: {
    handleClose() {
      // Không đặt lại các trường biểu mẫu để các lựa chọn trước đó được giữ lại khi chỉnh sửa
// Chỉ đặt lại trạng thái xác thực khi đóng hộp thoại
      if (this.$refs.knowledgeBaseForm) {
        this.$refs.knowledgeBaseForm.clearValidate();
      }
    ...
    loadRAGModels() {
      if (this.ragModels.length > 0) {
        return; // Đã tải rồi, tránh tải lặp lại
      }

      console.log('Bắt đầu tải danh sách mẫu RAG');
      Api.model.getRAGModels((res) => {
        console.log('RAGphản hồi danh sách mô hình:', res);
        if (res.data && res.data.code === 0) {
          this.ragModels = res.data.data || [];
          console.log('RAGĐã tải danh sách mẫu thành công, tổng cộng', this.ragModels.length, 'mô hình');

          // Nếu cơ sở kiến ​​thức mới được thêm vào và ragModelId không được đặt thì mô hình RAG đầu tiên sẽ được chọn theo mặc định.
          if (!this.form.id && !this.form.ragModelId && this.ragModels.length > 0) {
            this.$set(this.form, 'ragModelId', this.ragModels[0].id);
            console.log('Bộ mô hình RAG mặc định:', this.ragModels[0].id);
          }
        } else {
          console.error('Không lấy được danh sách model RAG:', res.data?.msg);
