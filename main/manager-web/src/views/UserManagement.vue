...
export default {
  ...
  methods: {
    ...
    resetPassword(row) {
      this.$confirm(this.$t('user.confirmResetPassword'), this.$t('common.warning'), {
        ...
        Api.admin.resetUserPassword(row.userid, ({ data }) => {
          if (data.code === 0) {
            // Hiển thị mật khẩu mặc định được tạo
    ...
    handleChangeStatus(row, status) {
      // Xử lý một người dùng hoặc một mảng người dùng
      ...
      this.$confirm(this.$t('user.confirmStatusChange', { action: actionText, count: userCount }), this.$t('common.warning'), {
        ...
        Api.user.changeUserStatus(status, userIds, ({ data }) => {
          if (data.code === 0) {
            ...
            this.fetchUsers(); // Làm mới danh sách người dùng
          } else {
            this.$message.error({
    ...
    // Phương thức này đã được thay thế bằng batchDelete và được dành riêng cho khả năng tương thích ngược
    handleBatchDelete() {
      this.batchDelete();
    },
