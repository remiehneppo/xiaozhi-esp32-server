  ...
  methods: {
    ...
    selectionCellClassName({ row, column, rowIndex, columnIndex }) {
      ...
      if (rowIndex === undefined) {
        // Sử dụng setTimeout để đảm bảo rằng DOM đã được hiển thị
        setTimeout(() => {
    ...
    handleMenuSelect(index) {
      ...
      this.pageSize = 10; // Tùy chọn: Đặt lại số mục trên mỗi trang
      this.loadData();
    },
    handleSearch() {
      this.currentPage = 1;
    ...
    handleModelSave({ provideCode, formData, done }) {
      ...
      if (this.editModelData.duplicateMode) {
        ...
        Api.model.addModel({ modelType, provideCode, formData }, ({ data }) => {
          ...
          done && done(); // Gọi lại cuộc gọi lại đã hoàn thành để đóng trạng thái tải
        });
      } else {
    ...
    // Xử lý các thay đổi trạng thái bật/tắt
    handleStatusChange(model) {
      ...
      Api.model.updateModelStatus(model.id, newStatus, ({ data }) => {
        if (data.code === 0) {
          ...
          // Khôi phục trạng thái ban đầu khi thao tác thất bại
...
::v-deep .el-table .custom-selection-header .cell::before {
  ...
  text-align: center;
  line-height: 32px;
  /* Đặt chiều cao dòng thích hợp để đảm bảo văn bản được hiển thị đầy đủ */
  color: black;
  margin-top: 0;
  /* Loại bỏ lề trên có thể gây ra sự bù đắp */
  height: 32px;
  position: absolute;
  top: 50%;
  left: 50%;
...
/* Các phần tử giả trống có thể ảnh hưởng đến hiển thị văn bản đã bị xóa */
...
.custom-pagination {
  ...
  /* Kiểu nút điều hướng (Trang chủ, trang trước, trang tiếp theo) */
