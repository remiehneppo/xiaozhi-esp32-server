...
/**
 * @description Vận hành cơ sở dữ liệu Triển khai dịch vụ cho bảng [ai_agent_plugin_mapping (bảng ánh xạ duy nhất giữa Tác nhân và trình cắm)]
    ...
    public List<AgentPluginMapping> agentPluginParamsByAgentId(String agentId) {
        ...
        for (int i = list.size() - 1; i >= 0; i--) {
            ...
            if (StringUtils.isBlank(mapping.getProviderCode())) {
                // Truy vấn các tham số plug-in cơ sở kiến thức
        ...
        if (knowledgeBaseMap.size() > 0) {
            for (String pluginCode : knowledgeBaseMap.keySet()) {
                ...
                String description = "Nếu người dùng hỏi với【"
                        + String.join(",", knowledgeBaseList.stream().map(KnowledgeBaseEntity::getName).toList())
                        + "】Phương pháp này nên được gọi khi nội dung liên quan đến phạm vi chủ đề được đề cập，cho truy vấn：" + String.join(",",
