        ...
        try:
            for file_name, file_path in model_files.items():
                if not os.path.isfile(file_path):
                    ...
                    if not os.path.isfile(file_path):
                        raise FileNotFoundError(f"mô hìnhtệpthất bại: {file_path}")

            self.model_path = model_files["model.int8.onnx"]
            self.tokens_path = model_files["tokens.txt"]

        except Exception as e:
            logger.bind(tag=TAG).error(f"mô hìnhtệpxử lý thất bại: {str(e)}")
            raise
    ...
    async def speech_to_text(
        ...
        try:
            ...
            logger.bind(tag=TAG).debug(
                f"giọng nóinhận dạngthời: {time.time() - start_time:.3f}s | kết quả: {text}"
            )
