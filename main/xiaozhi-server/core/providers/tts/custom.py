...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        if isinstance(self.params, str):
            try:
                ...
                raise ValueError("Custom TTScấu hìnhtham sốra,sẽký tựphân tíchchovới")
        elif not isinstance(self.params, dict):
            raise TypeError("Custom TTScấu hìnhtham sốra, cấu hìnhnói")
    ...
    async def text_to_speak(self, text, output_file):
        ...
        if resp.status_code == 200:
            ...
            error_msg = f"Custom TTSyêu cầuthất bại: {resp.status_code} - {resp.text}"
